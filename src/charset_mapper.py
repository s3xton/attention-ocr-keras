...
class CharsetMapper(object):
    ...
    def __init__(self, charset, max_sequence_length, null_char="\u2591", null_id=133):
        ...
        self.table_ids = tf.lookup.StaticHashTable(
            tf.lookup.KeyValueTensorInitializer(ids_tensor, chars_tensor, key_dtype=tf.int64, value_dtype=tf.string),
            null_char,
        )

        self.table_text = tf.lookup.StaticHashTable(
            tf.lookup.KeyValueTensorInitializer(chars_tensor, ids_tensor, key_dtype=tf.string, value_dtype=tf.int64),
            null_id,
        )
    ...
    def get_ids(self, text):
        ...
        char_seq = tf.concat([char_seq, [[self.null_char] * self.max_sequence_length]], axis=0)
